/**
 * Overlay NG-1 courtyard-feature into WorldRuntime ng1-i23d.
 * MERGE side container only. Never replace global.utoc.
 * Garden scaffold boxes stay out of UE. Not a world-model tavern.
 */
...
const COURTYARD_HASH =
  "6149953699316b57ad21210ef1334fa7bb86871c6bb62721b1e13a53e2ab4d70";
const COURTYARD_GLB = path.join(
  dataDir,
  "worlds/01M2AVSF55FF1J4SR3V0FPBC53.carina/assets",
  `${COURTYARD_HASH}.glb`,
);
const FEATURED = [
  ...
  {
    objectId: "courtyard-feature",
    assetId: COURTYARD_HASH.slice(0, 16),
    filename: "courtyard-feature.glb",
  },
] as const;
...
async function ssh(command: string, timeoutMs = 60_000): Promise<string> {
  const { stdout, stderr } = await execFileAsync(
    "ssh",
    ["-o", "ConnectTimeout=8", "carina-win", command],
    { timeout: timeoutMs },
  );
...
async function prepareCourtyard(assetId: string, assetHash: string): Promise<unknown> {
  const py =
    "from world_runtime.pipeline_prepare import prepare_asset; import json; " +
    `print(json.dumps(prepare_asset('${assetId}', '${assetHash}')))`;
  const raw = await ssh(
    `cd /d C:\\Users\\wuyw\\carina-rtx-validation && C:\\Users\\wuyw\\AppData\\Roaming\\uv\\python\\cpython-3.11.15-windows-x86_64-none\\python.exe -c "${py}"`,
    180_000,
  );
  const jsonStart = raw.indexOf("{");
  if (jsonStart < 0) {
    throw new Error(`prepare courtyard returned: ${raw.slice(0, 500)}`);
  }
...
async function main(): Promise<void> {
  ...
  const outPath = path.join(outDir, "ng1-viewport-extend.json");
  ...
  try {
    const courtyardBytes = await readFile(COURTYARD_GLB);
    const courtyardUpload = await client.upload({
      bytes: courtyardBytes,
      originalFilename: "courtyard-feature.glb",
      sourceLabel: "http-native-mesh",
      claimsWorldModelGeneration: false,
    });
    const prepared = await prepareCourtyard(courtyardUpload.assetId, courtyardUpload.assetHash);
